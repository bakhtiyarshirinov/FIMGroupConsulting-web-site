    ...
    return (
      <div className="p-8">
        <div className="flex items-center gap-4 mb-8">
          <button onClick={() => setMode("list")} className="text-gray-400 hover:text-white transition-colors">
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" />
            </svg>
          </button>
          <h1 className="font-playfair text-2xl font-bold text-white">
            {mode === "create" ? "Yeni xəbər əlavə et" : "Xəbəri düzəlt"}
          </h1>
        </div>
        ...
        <div className="max-w-3xl bg-dark-100 border border-gold/10 rounded-2xl p-8 space-y-6">
          ...
          <div>
            <label className={labelClass}>Məzmun *</label>
            <textarea
              rows={12}
          ...
          <div className="flex items-center gap-3">
            ...
            <span className="text-gray-300 text-sm font-inter">
              {form.published ? "Dərc edilib" : "Qaralama"}
            </span>
          </div>
  ...
  return (
    <div className="p-8">
      ...
      <div className="bg-dark-100 border border-gold/10 rounded-2xl overflow-hidden">
        {news.length === 0 ? (
          <div className="p-12 text-center text-gray-600 font-inter">Hələ xəbər əlavə edilməyib</div>
        ) : (
              ...
              {news.map((article) => (
                <tr key={article.id} className="hover:bg-dark-200 transition-colors">
                  ...
                  <td className="px-6 py-4">
                    ...
                    >
                      {article.published ? "Dərc edilib" : "Qaralama"}
                    </button>
                  </td>
                  <td className="px-6 py-4">
